...
if(imageInput != null){ // #imageInput 존재할 때
    ...
    const changeImageFn = e =>{
        ...
        // console.log(e.target.value); // 파일 경로 (fakepath 형태로 출력)
        ...
        if(uploadFile == undefined){ // 취소를 눌러서 files[0]에 파일이 없을 때
            ...
            // 1) backup한 요소를 복제 (요소는 한 번밖에 쓰지 못 하기 때문에 backup의 backup을 만듬)
            ...
            imageInput.addEventListener("change", changeImageFn); // 재귀호출
    ...
    // 1) 미리보기 -> 기본 이미지 변경
    // 2) input태그에 value 값을 빈 칸으로 변경(파일 없음)
    ...
    profileFrm.addEventListener("submit", e => {
        ...
        if(flag){ // flag가 true인 경우 수행
            e.preventDefault(); // form태그 제출 이벤트 막기(제거)
