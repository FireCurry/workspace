...
-- 이름, 직급코드, 급여 조회
SELECT EMP_NAME, JOB_CODE, SALARY, (SELECT AVG(SALARY) FROM EMPLOYEE SUB
				WHERE JOB_CODE = MAIN.JOB_CODE) "직급별 평균"
FROM EMPLOYEE MAIN
...
WHERE HIRE_DATE = /*(특정 부서 입사일이 가장 빠른 사람 SELECT)*/
	(SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB WHERE NVL(SUB.DEPT_CODE, 'XXX') = NVL(MAIN.DEPT_CODE, 'XXX') 
	AND ENT_YN != 'Y')
ORDER BY HIRE_DATE;

--(SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB WHERE (SUB.DEPT_CODE = MAIN.DEPT_CODE 
 ...
 * 	EX) 인라인뷰에서 지정된 별칭을 메인 쿼리에서도 똑같이 사용해야 된다
 * 
