...
WITH SELECT TRUNC((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'))/365) AS "만 나이"
SELECT EMP_ID, EMP_NAME, JOB_NAME, SELECT TRUNC((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'))/365) "만 나이",
	TO_CHAR(SALARY * 12 * (1 + NVL(BONUS, 0)), 'L999,999,999')
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE TRUNC((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'))/365) "만 나이";
