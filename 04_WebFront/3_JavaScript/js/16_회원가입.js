...
const checkList = {
  'input-id' : false, 'input-pw' : false, 'input-pw-check' : false,
  'input-name' : false
};
...
inputId.addEventListener('input', e=>{
  ...
  if(val.length === 0){ // 입력된 값이 없을 경우
    ...
    // 유효성 검사 여부 저장 객체에서
    // "input-id"의 값을 false로 변경 ('유효하지 않다'를 뜻함)
  ...
  if(regEx.test(val)){
    span.innerText = '유효한 아이디입니다.';
    span.classList.add('check');
    span.classList.remove('error');
    
    checkList['input-id'] = true;
  } else {
    span.innerText = '유효하지 않은 아이디입니다.'
...
//   // 양쪽 공백을 제거한 입력 값을 얻어와 val에 저장
//   const val = inputPw.value.trim();
//   const val2 = inputPwCheck.value.trim();
//   const span = e.target.nextElementSibling.nextElementSibling;
//   if(val.length === 0){
...
function checkPwFn(){
  ...
  if(pwVal.length === 0 || checkVal.length === 0){ // 비밀번호 미 입력 시
    span.innerText = '비밀번호를 먼저 입력 해주세요';
    span.classList.remove('check');
    span.classList.remove('error');
