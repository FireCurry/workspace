...
public class StudentView {
	...
	private void selectAllStudent() {
		...
		// Service로 부터 List<Student> 객체의 참조 주소를 받환받기
		...
		for (int i = 0 ; i < studentList.size() ; i++) {
			...
			System.out.printf("[%d] %s \n", i, studentList.get(i));
				...
				// print구문이라 문자로 출력해야 되어서 자동으로 toString() 추가해줌 Auto Unboxing 아님
		}
		
	}
	
	/**
	 * 인덱스가 일치하는 학생 1명 조회 view 메서드
	 */
	...
	private void deleteStudent() {
		...
		if (std == null) {
			...
			System.out.printf("%s 학생의 정보가 삭제되었습니다.", std.getName());
		}
	}
