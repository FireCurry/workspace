...
public class Server {
	...
	public void start() {
		...
		// 1. 서버의 포트 번호 정하기
		int port = 8500;
		// 포트번호는 0~65535 사이로 지정 가능 (2byte 범위)
		// 1023번 이하는 OS에서 사용하는 경우가 많으므로 사용을 권장하지 않는다
		
		// 2. 서버용 소켓 객체 생성
		
		// ServerSocket : 지정된 포트 번호로
		// 				클라이언트 연결이 되는 것을 기다리는 소켓 객체
		ServerSocket serverSocket = null; // 입장용 소켓
		
		Socket clientSocket = null; // 클라이언트와 연결되는 소켓
		
		InputStream is = null; // 클라이언트 -> 서버 스트림
		OutputStream os = null; // 서버 -> 클라이언트 스트림
