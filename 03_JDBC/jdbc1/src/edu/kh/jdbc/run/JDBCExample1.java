...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		try {
			...
			String userName = "a230724_kms_seok"; // 사용자 계정
			String pw = "01094292386"; // 비밀번호
			
			// DriverManager : Connection 만드는 객체
			conn = DriverManager.getConnection(type + host + port + dbName, userName, pw);
			// -> 어떤 타입의 어디에 있는 DB와 어떤 계정으로 연결할 지에 대한
			//		연결 방법을 저장한 Connection 객체 생성.,ㅣ
			
			// Connection 객체 색성 확인(오류 안 나면 오타 없이 작성한것
